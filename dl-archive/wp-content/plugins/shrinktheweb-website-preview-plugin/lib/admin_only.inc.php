...
function STWWT_showPage_Documentation()
{
	...
	<div id="stwwt_size_box">
		...
		<p>The Shrink The Web embed code shows a thumbnail for any link you wrap in <code>[thumb][/thumb]</code> tags. Any use of these tags is replaced
...
function STWWT_showPage_Settings()
{
	...
	define('STWWT_DESC_EMBEDDED', '
		<p>The Shrink The Web embed code shows a thumbnail for any link you wrap in <code>[thumb][/thumb]</code> tags. Any use of these tags is replaced
		with the thumbnail of the website included in the tags. <a href="admin.php?page=stwwt_documentation#embed">See some examples on the documentation page</a>.
	...
	$settingDetails = array(
		...
		'stwwt_break_embedded' => array(
				'type'  				=> 'break',
				'html'  				=> STWWT_forms_createBreakHTML('Embedded Screenshot Settings', 'Save ALL Settings') .
							   '<div class="stwwt_description">'.STWWT_DESC_EMBEDDED.'</div>',
			),
			
		'stwwt_embedded_default_size' => array(
				'label' 				=> 'Default Embedded Thumbnail Size',
				'type'  				=> 'select',				
				'data'					=> array(
												'mcr'	=> 'Micro (75x56)',
												'tny'	=> 'Tiny (90x68)',
				...
				'desc'  				=> 'The size of the thumbnail shown in embedded thumbnails in the thumbnail shortcode.',
				'default'				=> 'lg'
			),
		...
		'stwwt_break_embedded_pro' => array(
				'type'  				=> 'break',
				'html'  				=> STWWT_forms_createBreakHTML('Embedded Screenshot - Basic or PLUS Settings', 'Save ALL Settings') .
...
/*
// Old settings page - kept for migration checking.
function STWWT_showPage_Settings_OLD()
{
	...
	if ($_POST['updatingstw']) {
		if ($_POST['stw_keyid']!='') {
			$good			=	true;
			$stw_enable		=	$_POST['stw_enable'];
			$stw_keyid		=	$_POST['stw_keyid'];
			$stw_sz			=	$_POST['stw_sz'];
			$stw_embedsz	=	$_POST['stw_embedsz'];						// ON even if popups disabled
			$stw_permspec	=	$_POST['stw_permspec'];
			$stw_permfull	=	$_POST['stw_permfull'];
			$stw_permqual	=	$_POST['stw_permqual'];
			$stw_stwq		=	$_POST['stw_stwq'];
			$stw_method		=	$_POST['stw_method'];
		} else {
			$stw_enable		=	false;
			$good			=	false;
		}
		update_option('STWThumbnails', $stw_enable);
		update_option('STWThumbnails_KeyId', $stw_keyid);
		update_option('STWThumbnails_sz', $stw_sz);
		update_option('STWThumbnails_embedsz', $stw_embedsz);
		update_option('STWThumbnails_permspec', $stw_permspec);
		update_option('STWThumbnails_permfull', $stw_permfull);
		update_option('STWThumbnails_permqual', $stw_permqual);
		update_option('STWThumbnails_stwq', $stw_stwq);
		update_option('STWThumbnails_method', $stw_method);

		if($good){
			echo '<div id="message" class="updated fade"><p>STW Thumbnails Options saved successfully.</p></div>';
		} else {
			echo '<div id="message" class="updated fade"><p>There is missing information. Please check and try again.</p></div>';
		}
	}

?>
<div class="wrap">
<h1><?php _e('Shrink The Web Thumbnails')?></h1>
<p><a href="http://www.shrinktheweb.com/user/register" target="_blank"><img src="http://www.shrinktheweb.com/uploads/stw-banners/shrinktheweb-234x60.gif" alt="Website Thumbnail Provider" border=0 width="234" height="60" align="left" hspace="10" style="padding-right:10px;"></a> <b>Need an account? Register for FREE</b><br />
<?php _e('Enable STW Website Thumbnails for your blog.')?><br />
<a href="http://www.shrinktheweb.com" target="_blank">www.shrinktheweb.com</a></font></p>
<form action="" method="post">
<h2 style="padding-top:10px;">STW Credentials</h2>
<p><label><input name="stw_keyid" id="stw_keyid" value="<?php echo $stw_keyid ?>" type="text" /> Access Key Id</label></p>
<h2 style="padding-top:10px;">Mouseover Popup Settings</h2>
<p><label><input name="stw_enable" id="stw_enable" value="yes" type="checkbox" <?php if ($stw_enable) { ?> checked="checked" <?php } ?> /> Enable the <a href="http://www.shrinktheweb.com/" target="_blank">STW Website Thumbnails</a> mouseover popups Plugin on this blog.</label></p>
<p><select name="stw_sz" id="stw_sz">
<option <?php if($stw_sz == 'sm' || '') { echo 'selected'; } ?> value="sm">Small (120x90)</option>
<option <?php if($stw_sz == 'lg') { echo 'selected'; } ?> value="lg">Large (200x150)</option>
<option <?php if($stw_sz == 'xlg') { echo 'selected'; } ?> value="xlg">x-Large (320x200)</option>
</select> Choose Preview Bubble Size</p>
<select name="stw_method" id="stw_method" style="margin-top:10px;">
<option <?php if($stw_method == 'automatic' || '') { echo 'selected'; } ?> value="automatic">Automatic</option>
<option <?php if($stw_method == 'manual') { echo 'selected'; } ?> value="manual">Manual</option>
</select> <div style="padding:5px 0 10px 0;">If "<b>Automatic</b>" is selected, all external links will show ShrinkTheWeb preview bubbles.<br />Use <code>class="nopopup"</code> to disable popup bubble for a specific link.<br/><br />
If "<b>Manual</b>" is selected, then you choose which links get a preview bubble by adding<br/>
<code>class="stwpopup"</code> to any link where you want to show them.</div>
<h2 style="padding-top:10px;">Embedded Screenshot Settings</h2>
<div style="padding:5px 0 10px 0;">Usage: In your post, click 'HTML' and type: [thumb]http://www.domain.com[/thumb]</div>
<p><select name="stw_embedsz" id="stw_embedsz">
<option <?php if($stw_embedsz == 'mcr' || '') { echo 'selected'; } ?> value="mcr">Micro (75x56)</option>
<option <?php if($stw_embedsz == 'tny') { echo 'selected'; } ?> value="tny">Tiny (90x68)</option>
<option <?php if($stw_embedsz == 'vsm') { echo 'selected'; } ?> value="vsm">Very Small (100x75)</option>
<option <?php if($stw_embedsz == 'sm' || '') { echo 'selected'; } ?> value="sm">Small (120x90)</option>
<option <?php if($stw_embedsz == 'lg') { echo 'selected'; } ?> value="lg">Large (200x150)</option>
<option <?php if($stw_embedsz == 'xlg') { echo 'selected'; } ?> value="xlg">x-Large (320x200)</option>
</select> Choose Embedded Screenshot Size</p>
<h3 style="padding-top:10px;">STW PRO Features (requires upgrade)</h2>
<p><label><input name="stw_permspec" id="stw_permspec" value="1" type="checkbox" <?php if ($stw_permspec) { ?> checked="checked" <?php } ?> /> Specific "Inside" Pages</label></p>
<p><label><input name="stw_permfull" id="stw_permfull" value="1" type="checkbox"  <?php if ($stw_permfull) { ?> checked="checked" <?php } ?> /> Full-length Captures</label></p>
<p><label><input name="stw_permqual" id="stw_permqual" value="1" type="checkbox"  <?php if ($stw_permqual) { ?> checked="checked" <?php } ?> />
		  <input name="stw_stwq" id="stw_stwq" value="<?php if($stw_stwq && $stw_permqual){echo $stw_stwq;}else{echo "98";} ?>" type="text" maxlength="3" size="3" />% Custom Quality</label></p>
<p class="submit">
<input value="Update &raquo;" type="submit" />
<input name="updatingstw" value="true" type="hidden" /></p>
</form>
<p>&nbsp;</p>
<p>Author: <a href="http://www.neosys.net/profile.htm" target="_blank">Neosys Consulting, Inc.</a></p>
</div>
<?php
}
*/
